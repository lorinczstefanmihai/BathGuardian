...
void aws_iot_task(void *param) {
    ...
    // Debug: log certificate lengths and start of data
    size_t root_ca_len = (const char *)aws_root_ca_pem_end - (const char *)aws_root_ca_pem_start;
    size_t cert_len = (const char *)certificate_pem_crt_end - (const char *)certificate_pem_crt_start;
    size_t key_len = (const char *)private_pem_key_end - (const char *)private_pem_key_start;
    
    ESP_LOGI(TAG, "Root CA length: %d, Device Cert length: %d, Private Key length: %d", 
             root_ca_len, cert_len, key_len);
    ...
    mqttInitParams.isSSLHostnameVerify = false;  // Disable hostname verification temporarily for debugging
    ...
    while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)) {
        ...
        sprintf(cPayload, "%s : %.1f, %s : %.1f", "MQ135-raw", getMQ135raw(), "MQ135-volt", getMQ135volt());
        paramsQOS0.payloadLen = strlen(cPayload);
        rc = aws_iot_mqtt_publish(&client, TOPIC, TOPIC_LEN, &paramsQOS0);

        sprintf(cPayload, "%s : %.1f, %s : %.1f", "Temperature", getDHT11Temperature(), "Humidity", getDHT11Humidity());
