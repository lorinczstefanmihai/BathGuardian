...
/**
 * Gets DHT11 sensor temperature and humidity values for display on the webpage
 */
...
/**
 * Sets interval to get DHT11 sensor values every 1 second
 */
 ...
 * Gets DHT11 sensor temperature and humidity values for display on the webpage
 */
function getMQSensorValues()
{
    $.getJSON('/mqSensor.json', function(data){
        $('#raw_reading').text(data["raw"]);
        $('#volt_reading').text(data["volt"]);
    });
}

/**
 * Sets interval to get MQ135 sensor values every 1 second
 */
function startMQSensorInterval()
{
    setInterval(getMQSensorValues, 1000);
}
...
/**
 * Disconnects WiFi once the disconnect button is pressed and reloads the page.
 */
