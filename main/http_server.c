...
static TaskHandle_t http_server_monitor_handle = NULL;
...
// Embedded files: JQuery, index.html, app.js and favicon.ico files
        ...
        if (xQueueReceive(http_server_monitor_queue_handle, &msg, portMAX_DELAY))
        {
            switch (msg.msg_id)
            {
                case HTTP_MSG_WIFI_CONNECT_INIT:
                    ESP_LOGI(TAG, "Monitor: WiFi connection initiated");

                    g_wifi_connect_status = HTTP_WIFI_STATUS_CONNECTING;

                    break;

                case HTTP_MSG_WIFI_CONNNECT_SUCCESS:
                    ESP_LOGI(TAG, "Monitor: WiFi connection successful");
                ...
                case HTTP_MSG_WIFI_CONNECT_FAIL:
                    ESP_LOGI(TAG, "Monitor: WiFi connection failed");
				...
				case HTTP_MSG_OTA_UPDATE_SUCCESSFUL:
                    ESP_LOGI(TAG, "Monitor: OTA update successful");
                    g_fw_update_status = OTA_UPDATE_SUCCESSFUL;
                    http_server_fw_update_reset_timer();
                    break;

                case HTTP_MSG_OTA_UPDATE_FAILED:
                    ESP_LOGI(TAG, "Monitor: OTA update failed");
                    g_fw_update_status = OTA_UPDATE_FAILED;
                    break;

                default:
                    ESP_LOGW(TAG, "Monitor: Unknown message ID: %d", msg.msg_id);
                    break;
            }
        }
    }
...
/**
 * JQuery get handler is requested when accesing the webpage
 * @param req HTTP request for which the uti needs to be handled
 * @return ESP_OK
 */
...
/**
 * app.css get handler is requested when accesing the webpage
 * @param req HTTP request for which the uti needs to be handled
 * @return ESP_OK
 */
...
/**
 * app_js get handler is requested when accesing the webpage
 * @param req HTTP request for which the uti needs to be handled
 * @return ESP_OK
 */
static esp_err_t http_server_app_js_handler(httpd_req_t *req)
{
    ESP_LOGI(TAG, "app_js requested");
...
/**
 * Sends the favicon.ico file when accesing the webpage
 * @param req HTTP request for which the uti needs to be handled
 * @return ESP_OK
 */
...
static esp_err_t http_server_get_dht_sensor_readings_json_handler(httpd_req_t *req)
{
	...
	sprintf(dhtSensorJSON, "{\"temp\":\"%.1f\",\"humidity\":\"%.1f\"}", getDHT11Temperature(), getDHT11Humidity());
...
/**
 * MQ sensor readings JSON handler responds with MQ135 sensor data
 * @param req HTTP request for which the uri needs to be handled
 * @return ESP_OK
 */
static esp_err_t  http_server_get_mq_sensor_readings_json_handler(httpd_req_t *req)
{
    ESP_LOGI(TAG, "/mqSensor.json requested");

	char mqSensorJSON[100];

	sprintf(mqSensorJSON, "{\"raw\":\"%.1f\",\"volt\":\"%.1f\"}", getMQ135raw(), getMQ135volt());

	httpd_resp_set_type(req, "application/json");
	httpd_resp_send(req, mqSensorJSON, strlen(mqSensorJSON));
...
/**
 * wifiConnectInfo.json handler updated the web page with connection information
 * @param req HTTP request for which the uri needs to be handled.
 * @return ESP_OK
 */
 ...
 * @return http server instance handle if succesful, NULL otherwise
 */
static httpd_handle_t http_server_configure(void)
{
    ...
    //The core that the HTTP server task will run on
    ...
    ESP_LOGI(TAG, "http_server_configure: Starting HTTP server on port: %d with task priority: %d", config.server_port, config.task_priority);
    ...
    if(httpd_start(&http_server_handle, &config) == ESP_OK)
    {
        ...
        httpd_uri_t dhtSensor_json = {
            .uri       = "/dhtSensor.json",
            .method    = HTTP_GET,
            .handler   = http_server_get_dht_sensor_readings_json_handler,
            .user_ctx  = NULL
        };
        httpd_register_uri_handler(http_server_handle, &dhtSensor_json);

        //register mqSensor.json handler
        httpd_uri_t mqSensor_json = {
            .uri       = "/mqSensor.json",
            .method    = HTTP_GET,
            .handler   = http_server_get_mq_sensor_readings_json_handler,
            .user_ctx  = NULL
        };
        httpd_register_uri_handler(http_server_handle, &mqSensor_json);
...
void http_server_stop(void)
{
    if(http_server_handle)
    {
        ...
        ESP_LOGI(TAG, "http_server_stop: HTTP server stopped");
    http_server_handle = NULL;
    }

    if(http_server_monitor_handle)
    {
        vTaskDelete(http_server_monitor_handle);
        http_server_monitor_handle = NULL;
        ESP_LOGI(TAG, "http_server_stop: HTTP server monitor task stopped");
    }
}

BaseType_t http_server_monitor_send_message(http_server_msg_e msg)
{
    http_server_queue_message_t message;
    message.msg_id = msg;
