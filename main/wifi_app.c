...
static void wifi_app_task(void *pvParameters)
{
    ...
    wifi_app_send_message(WIFI_APP_MSG_LOAD_SAVED_CREDIANTIALS);
    ...
    while (1)
    {
        ...
        if (xQueueReceive(wifi_app_queue_handle, &msg, portMAX_DELAY))
        {
            switch (msg.msgID)
            {
                case WIFI_APP_MSG_LOAD_SAVED_CREDIANTIALS:
                    ESP_LOGI(TAG, "Loading saved WiFi credentials from NVS...");
                ...
                case WIFI_APP_MSG_STA_DISCONNECTED:
					...
					else
					{
						...
						// Adjust this case if I want to keep trying to connect...
					}
